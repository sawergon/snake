...
TEST(Constructor_param, test_2) { ASSERT_ANY_THROW(S21Matrix M(-2, 2)); }
...
TEST(Constructor_moving, test_1) {
  ...
  EXPECT_EQ(m1.get_raw(), nullptr);
  EXPECT_NE(m2.get_raw(), m3.get_raw());
...
TEST(Operator_NotEqMatrix, test_1) {
  S21Matrix M(3, 3);
  S21Matrix M2(2, 2);
  for (int i = 0; i < M.get_rows(); i++) {
    for (int j = 0; j < M.get_cols(); j++) {
...
TEST(Operator_NotEqMatrix, test_2) {
  S21Matrix M(3, 3);
  S21Matrix M2(3, 3);
  for (int i = 0; i < M.get_rows(); i++) {
    for (int j = 0; j < M.get_cols(); j++) {
...
TEST(Operator_NotEqMatrix, test_3) {
  S21Matrix M(3, 3);
  S21Matrix M2(3, 3);
  M(0, 0) = 123;
  M(0, 1) = 123;
...
TEST(Operator_NotEqMatrix, test_4) {
  S21Matrix M;
  S21Matrix M2;
  ASSERT_EQ(M != M2, 1);
}
...
TEST(S21MatrixTest, MultiplicationOperator) {
